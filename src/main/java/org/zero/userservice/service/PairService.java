...
public class PairService {
    ...
    public String createPair(
        ...
        var idempotencyCheckValue = IdempotencyValueProvider.generate(pair.userId(), pair.warehouseRef());
        var idempotencyCheckResult = idempotencyProvider.check(idempotencyKey, idempotencyCheckValue);

        if (idempotencyCheckResult.exist()) {
            if (idempotencyCheckResult.equals()) {
                var foundedPair = repository.findFirstByContactPerson_IdAndWarehouseRef(contactPersonId, pair.warehouseRef());
        ...
        var newPair = getContactWarehousePair(pair, contactPersonId, pairHash);
        return savePair(idempotencyKey, newPair, idempotencyCheckValue);
    }



    public PairResponse createContactPersonAndGetPair(
            FullPairRequest pair,
            String idempotencyKey,
            String issuerId
    ) {
        ...
        var createdContactPerson = userService.createContactPerson(contactPerson, idempotencyKey);

        return new PairResponse(contactPersonId, warehouseRef, createdContactPerson.userId());
    }
    ...
    private String savePair(
            String idempotencyKey,
            ContactWarehousePair newPair,
            String idempotencyCheckValue
    ) {
    ...
    private ContactWarehousePair getContactWarehousePair(
        ...
        newPair.setHash(pairHash);
        return newPair;
    }


    private void checkIssuerPermissionToCreatePair(
            String issuerId,
            Integer contactPersonId
    ) {
        var parsedIssuerId = uuidProvider.get(issuerId);
    ...
    private ContactWarehousePair getPairOrThrow(
            FullPairRequest pair
    ) {
    ...
    private static boolean isContactPersonIssuerAndIssuerEquals(
            Optional<ContactPerson> contactPerson,
            Integer parsedIssuerId
    ) {
        return !Objects.equals(contactPerson.get().getIssuerUser().getId(), parsedIssuerId);
    }

    private static boolean isContactPersonIssuerAndIssuerEquals(
            ContactWarehousePair foundedPair,
            Integer parsedIssuerId
    ) {
    ...
    private static boolean isExistAndHashEquals(
            Optional<ContactWarehousePair> foundedPair,
            String pairHash
    ) {
