...
public class UserController {
    ...
    public List<UserDataResponse> getByPhone(
        ...
        if (user.isEmpty()) throw new RequestException("Contact person not founded");
        return user.stream().map(element -> new UserDataResponse(element.getFirstName(), element.getLastName(), element.getMiddleName(), element.getPhone(), issuerId)).toList();
    }
}
